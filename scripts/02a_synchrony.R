...
pca_scores <- read.csv("output_data/01a_trait_pcoa_scores_new_sites_new_ord.csv")
head(pca_scores)
pca_scores <- pca_scores %>%
  rename(Axis1 = X1, Axis2 = X2)
## which axis?
## do not know which dimension is what - change later
...
for (basin in 1:length(basinsID)) {
  ...
  for (ax in 1: length(Naxis)) {
    ...
    # head(correlation)
    write.csv(correlation, paste("output_data/cor_matrix/02a_site_sync_", basinsID[basin], 
                                 "_", Naxis[ax], ".csv", sep=""))
...
###save results
write.csv(synchrony_axis, "output_data/02a_results_between_site_synchrony_new_ord_new_sites.csv")
...
###save results
write.csv(synchrony_axis, "output_data/02a_results_between_site_synchrony_new_sites_new_ord_no_boundaries.csv")
# synchrony_axis <- read.csv("output_data/02_results_between_site_synchrony.csv")
synchrony_axis
## plot synchrony
...
pca_scores <- pca_scores %>%
  rename(site_year = X, Axis1 = X1, Axis2 = X2)
...
write.csv(synchrony_axis, "output_data/02a_between_basin_sync_per_country_new_ord.csv")  
...
write.csv(synchrony_axis, "output_data/02a_between_basin_sync_all_together_new_ord.csv")  
...
#  synchrony flow temp no boundaries --------------------------------------
...
Nvar <- unique(clim_sites$env_var)
...
ax=1
  ...
  for(ax in 1: length(Nvar)) {
    ...
    axis_data<-clim_sites[clim_sites$env_var==unique(clim_sites$env_var)[ax],]
    years <- unique(sort(axis_data$year)) ## define years for columns

    # make df wide
    axis_data  <- axis_data %>% 
      dplyr::select(-c(HydroBasin, Country) ) %>%
      spread(SiteID, Temp) #%>%
    ...
    correlation<-cor(t(axis_data), use = "pairwise.complete.obs")
    head(correlation)
    write.csv(correlation, paste("output_data/cor_matrix/02a_temp_site_sync_", 
                                 "_", Nvar[ax], ".csv", sep=""))
    ...
    site_ID2<-sapply(strsplit(names(correl_result),":"),'[',2)
    # correl_result
    synchrony_axis<-rbind(synchrony_axis, cbind(as.character(unique(clim_sites$env_var)[ax]),
                                                correl_result,site_ID1,site_ID2))
...
warnings()
synchrony_axis<-data.frame(synchrony_axis)
colnames(synchrony_axis)<-c( "Env_Var","Correlation","Site_ID1","Site_ID2")
nlevels(factor(synchrony_axis$Country)) # 43
nlevels(factor(synchrony_axis$Env_Var)) # 3
range(synchrony_axis$Correlation)

write.csv(synchrony_axis, "output_data/02a_temp_sync_between_sites_new_ord_no_boundaries.csv")
...
Nvar <- unique(flow_sites$env_var)
...
for(ax in 1: length(Nvar)) {
  ...
  axis_data<-flow_sites[flow_sites$env_var==unique(flow_sites$env_var)[ax],]
  years <- unique(sort(axis_data$year)) ## define years for columns
  ...
  correlation<-cor(t(axis_data), use = "pairwise.complete.obs")
  head(correlation)
  write.csv(correlation, paste("output_data/cor_matrix/02a_flow_site_sync_", 
                               "_", Nvar[ax], ".csv", sep=""))
  ...
  site_ID2<-sapply(strsplit(names(correl_result),":"),'[',2)
  # correl_result
  synchrony_axis<-rbind(synchrony_axis, cbind(as.character(unique(flow_sites$env_var)[ax]),
                                              correl_result,site_ID1,site_ID2))
...
write.csv(synchrony_axis, "output_data/02a_flow_sync_between_sites_new_ord_no_boundaries.csv")
